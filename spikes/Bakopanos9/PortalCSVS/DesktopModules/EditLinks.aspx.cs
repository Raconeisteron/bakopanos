...
namespace ASPNET.StarterKit.Portal
{
    ...
    {
        [Dependency]
        public ILinkDB LinkDB { private get; set; }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Page.IsPostBack == false)
            {
                if (itemId != 0)
                {
                    ...
                    IDataReader dr = LinkDB.GetSingleLink(itemId);
        ...
        protected void UpdateBtn_Click(Object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                if (itemId == 0)
                {
                    ...
                    LinkDB.AddLink(moduleId, itemId, Context.User.Identity.Name, TitleField.Text, UrlField.Text, MobileUrlField.Text, Int32.Parse(ViewOrderField.Text), DescriptionField.Text);
                }
                else
                {
                    // Update the link within the Links table
                    LinkDB.UpdateLink(moduleId, itemId, Context.User.Identity.Name, TitleField.Text, UrlField.Text, MobileUrlField.Text, Int32.Parse(ViewOrderField.Text), DescriptionField.Text);
                }
